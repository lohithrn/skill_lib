...
```jsonc
{
  ...
  "tools": {
    ...
    "caps":       {"command": "bash /Users/me/.claude/skills/codegraph/scripts/caps.sh --json --root .",
                   "runs": true, "baseline": "file_lines_major=12", "seconds": 6},
    "graph":      {"command": "bash /Users/me/.claude/skills/codegraph/scripts/graph.sh --json --root .",
...
| `command` holds a resolved **absolute** path, never `${CLAUDE_PLUGIN_ROOT}` or `${CLAUDE_SKILL_DIR}` | `SKILL.md` §Scripts resolves the skill directory once, in phase 0, by trying `${CLAUDE_PLUGIN_ROOT}/skills/codegraph`, then `${CLAUDE_SKILL_DIR}`, then the directory holding `SKILL.md`. This file records the answer. A variable is portable only inside the process that sets it, and this string is executed by other processes; that is also why phase 0 runs it once to set `runs` — the path is proven, not assumed |
...
Missing file ⇒ every one of them stops and says "run `/codegraph analyze` first". None of them
invents a command.
