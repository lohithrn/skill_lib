...
allowed-tools: Read, Grep, Glob, Write, Edit, TodoWrite, Agent, Bash(mkdir:*), Bash(git log:*), Bash(git status:*), Bash(git diff:*), Bash(git rev-parse:*), Bash(git ls-files:*), Bash(git add:*), Bash(git commit:*), Bash(git checkout:*), Bash(git switch:*), Bash(git revert:*), Bash(bash ${CLAUDE_PLUGIN_ROOT}/skills/md_codegraph/scripts/*), Bash(bash ${CLAUDE_SKILL_DIR}/scripts/*)
---
...
`.codegraph/oracle.json`.** Try `${CLAUDE_PLUGIN_ROOT}/skills/md_codegraph`, then `${CLAUDE_SKILL_DIR}`,
then the directory holding this `SKILL.md`; first one with a `scripts/caps.sh` wins. Later phases
and subagents run the recorded string — an unexpanded `${...}` becomes `bash /skills/…` elsewhere.
