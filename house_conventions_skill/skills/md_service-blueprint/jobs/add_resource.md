...
1. **Read the repo's `infra/terraform/naming.tf`** (or, in an older repo, how it composes names inline and
   ...
   disagree, the repo wins for that repo, and the `md_fleet-conventions` skill says the same.
...
4. **Add exactly one `locals` entry** (or one inline composition, matching the repo) named
   `<prefix>_<resource_role>_<company>`. Read `references/naming.md` before choosing the `resource_role`:
