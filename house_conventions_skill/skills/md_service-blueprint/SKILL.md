---
name: md_service-blueprint
description: The house style for building and deploying an AWS lambda service — the repository layout, the <prefix>_<resource_role>_<company> naming convention composed in one naming.tf, the self-contained deploy script and its nine ordered gates, the urls.{env}.yaml outbound-URL seam, and the eight working rules. Ships the naming, deploy, account-config and URL-resolver templates it copies. Use when creating a new service or repo, adding infra or a resource, writing or changing a deploy script, naming any AWS resource, wiring outbound URLs or per-environment config, or checking a service for production readiness.
when_to_use: Standing up a new service, repo, backend or frontend from nothing; adding a resource, route or deploy change to one that exists; or reviewing a service against the house style before a deploy.
...
to the `md_fleet-conventions` skill. This one is the job of building.
...
| naming *any* AWS resource, tfvars var or env var | `references/naming.md` |
| wiring an outbound URL or per-environment config | `references/deploy_discipline.md` |
| "is this production ready?" / review before a deploy | `references/working_rules.md`, then the checklist below |
...
- **It does not review an existing fleet against the standing rules** — that is the `md_fleet-conventions`
  skill, which also holds the measured name-length evidence and the index of which repo demonstrates what.
...
| `references/working_rules.md` | the eight working rules, the consequence attached to each, and where the ninth lives |
| `references/naming.md` | how a name is composed, the taste rules, the three 64-character namespaces and their slug variables, and the plan-time assertion |
...
| `references/repo_layout.md` | the full tree with per-file responsibilities, the rules of thumb, the code-size caps, and the cost defaults |
