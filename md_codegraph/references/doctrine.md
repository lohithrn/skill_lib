...
6. **`src/` is the running system and nothing else.** Only what executes in production and what
   the deploy needs lives under `src/`: the entrypoints, the resolvers, the ports, the utils
   floor. Tests, QE harnesses, fixtures, generators, experiments, scratch scripts and one-off
   tooling live in sibling top-level trees — `tests/`, `scripts/`, `local_development/`. A
   non-runtime file under `src/` is a node in the graph that no request ever reaches, so it
