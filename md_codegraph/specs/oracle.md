...
```jsonc
{
  ...
  "tools": {
    ...
    // ABSOLUTE, resolved once in phase 0. Not `${CLAUDE_SKILL_DIR}`: this string is executed
...
| `command` holds a resolved **absolute** path, never `${CLAUDE_SKILL_DIR}` | `SKILL.md` §Scripts resolves the skill directory once, in phase 0, by trying `${CLAUDE_SKILL_DIR}`, then the directory holding `SKILL.md`. This file records the answer. A variable is portable only inside the process that sets it, and this string is executed by other processes; that is also why phase 0 runs it once to set `runs` — the path is proven, not assumed |
