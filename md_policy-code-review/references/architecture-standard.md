# The architecture standard — R1–R40

The mandatory architectural code-review, dependency-injection, maintainability and refactoring
standard: **40 numbered rules** for reviewing, refactoring or writing a codebase where
maintainability, encapsulation, DI, small files, small methods, interface-driven design, polymorphism
and dead-code cleanup matter. Cited as policy group `R` (`R1`–`R40`) — a suggestion citing `R4` means
rule 4 of this standard, whose full text is in the reference file named in §Route below.
...
branch becomes an interface) and the three brakes (rules 28–30). Without it this standard turns a
...
- In `refactor` mode the write tools are available because rules 24–25 apply edits. What stops an edit
  is this gate, not a missing tool: no inspection and no plan ⇒ no edit. In `review` and `diff` mode
  nothing is edited at all — see SKILL.md §The gate.
...
These are mandatory review standards, not defaults. When this standard is invoked, **apply every
relevant rule as written.** Do not silently weaken thresholds, substitute personal preferences, or skip
a rule because the existing codebase violates it. **Clearly identify any justified exception rather than
inventing one.**
...
| what must my refactor report contain? | `specs/review-output.md` |
...
6. **Guard clauses stay.** Null, empty, validation, permission, boundary and early-return checks are
   not the branching this standard hunts. Do not blindly replace every `if`.
...
`md_codegraph` is the descendant of this standard: this states the rules, codegraph **measures** them at
repo scale. Where a rule here is a number — 250-line files, 25-line methods, 8-line loop bodies, one
...
the three caps and stops there: it is the pre-pass for one review, not a graph. Use group `R` to decide
what is right and to refactor; use codegraph when you need the numbers proven rather than asserted. The
rules in `references/public-api.md` and `references/data-and-wiring.md` have no counterpart there.
...
- **It does not rule on correctness, security, or performance.** A god class full of correct logic is
  in scope; a race condition inside a 20-line method is not — that is group `C`
...
## Checklist before saying done — `refactor` mode
