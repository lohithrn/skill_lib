...
POLICY       the one policy ID this cites — G1..G10 | H1..H12 | C1..C6
...
## Verdict            one line, computed per §5, plus the arithmetic
## Groups run         which of G/H/C ran, on what file set, and why the others did not
...
```
Policy review: <repo> — DRIFTING. 2 blockers, 9 majors, 17 minors. Nothing was changed.
...
Top 3
1  PCR-H02-001 blocker S  naming.tf:41 role name 78 chars — apply fails; add var.role_slug
2  PCR-C01-002 blocker S  handlers/create.py:88 tenant compared before auth gate returns
3  PCR-G07-004 major  M  commons/dynamo.py:120 except: pass — 3 swallowed, no stack in logs

Full: .policy-review/report.md
Next: fix the two blockers (both S), then re-run /policy-code-review diff
```
