# The scaffold and packaging standard

The arc-1 rulebook: create, audit or repair an in-house Python library repository so the shared
packaging pattern survives. Cited from `SKILL.md` as **arc 1 — scaffold**.
...
| Publishing artifacts and a baked manifest as well | arc 2 — `references/lifecycle-standard.md` |

Arc 2 (`references/lifecycle-standard.md`) is a **superset** for one kind of library: it assumes everything here plus a
...
- **It does not describe the artifact lifecycle.** Baked manifests, artifact buckets, mounts and
  tool-calling belong to arc 2, `references/lifecycle-standard.md`. A library can follow every rule here and
