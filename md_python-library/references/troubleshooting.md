...
| `RuntimeError` naming two model or version ids | An artifact was built with one model and is being read with another — not a migration error at all | See arc 2 (`references/lifecycle-standard.md`): the build-time and query-time ids must be equal |
