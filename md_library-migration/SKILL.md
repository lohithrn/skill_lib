---
name: md_library-migration
description: Write, stage and verify an old-to-new import map when a shared Python library's public surface is refactored — a monolithic commons/utils package split into domain modules, free functions turned into injected instances, environment-variable reads hoisted out of the library, bundled dependencies turned into extras. Covers how to build the map from measurement, the order to migrate a consumer in, how to prove no old import is left behind, and when a backward-compatible shim may be removed. Use when planning or executing a breaking rename of a library's import paths, or when reviewing someone else's migration guide.
when_to_use: A library's public import paths are changing and other repos import them.
